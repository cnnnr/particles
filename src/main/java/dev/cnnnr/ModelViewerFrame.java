...
class ModelViewerFrame extends JFrame
{
	...
	private JPanel buildStyleEditor()
	{
		...
		windZSpinner.addChangeListener(e -> saveStyle());
		windZSpinner.setToolTipText("Steady vertical drift in local units/sec. Positive is up, negative is down (like Rise, but a constant that drag does not decay). Updrafts, sinking mist.");
		...
		vortexSpinner.addChangeListener(e -> saveStyle());
		vortexSpinner.setToolTipText("Radial speed from the emitter's vertex centre, local units/sec. Positive blows particles outward (a burst), negative sucks them inward (an implosion). Needs a vertex ring - no effect on single-point targets.");
		emitScaleSpinner.addChangeListener(e -> saveStyle());
	...
	void refreshStyleEditor()
	{
		...
		windZSpinner.setValue(profile.getWindZ());
		dragSpinner.setValue(profile.getDrag());
		vortexSpinner.setValue(profile.getVortex());
	...
	private void saveStyle()
	{
		...
		profile.setWindZ((int) windZSpinner.getValue());
		profile.setDrag((int) dragSpinner.getValue());
		profile.setVortex((int) vortexSpinner.getValue());
